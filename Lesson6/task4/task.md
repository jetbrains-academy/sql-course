In this exercise, you will practice using **Common Table Expressions** (CTEs) via the `WITH` clause to query the Astrofleet tables:
...
### Task
Read the task definitions in `task.sql` and replace each highlighted fragment so that each query returns the
required result.

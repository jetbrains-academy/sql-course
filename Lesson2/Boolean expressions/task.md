...
> **Note.** SQLite, the engine used in this course, has no dedicated boolean type: it prints
> **`1`** for true and **`0`** for false. The keywords `true` and `false` are accepted, but they
> are just aliases for `1` and `0`.

### Comparison operators

Using comparison operators, we can compare values of the same type.

The equality operator in SQL is a single **`=`** (which again may surprise programmers who are used to

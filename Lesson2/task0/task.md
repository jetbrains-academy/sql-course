...
In this lesson we will take a closer look at SQL queries and will learn how to write a query
which finds all rows of a table which meet some criteria. 

## Using SQL as a calculator

First we will learn how to calculate expressions in SQL.
...
of an arithmetical expression. It does not access any table yet.

```sql
SELECT (3+4)*6;
```

Arithmetical operations and parentheses work as you may expect. There is a number of built-in 
...
name is not specified, it is generated by the SQL engine. However, it is possible to specify column
names explicitly using a keyword `AS`:

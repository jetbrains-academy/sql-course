This exercise has **two** queries. Each contains a `?` placeholder that the
checker fills with different values — keep the `?` in every query.

### Task
**Query 1 — `first_result`:** change the expression so it is true only when the
value equals the string `'the moon'`, ignoring the letter case.

**Query 2 — `second_result`:** change the expression so it is true only when the
value is the square of an integer from 2 to 10 inclusive. Hint: use the `IN` operator.

Edit `task.sql` and click **Check**.

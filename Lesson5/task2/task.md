In this exercise, you will practice using aggregate functions (such as `COUNT` and `AVG`), `DISTINCT`, and scalar subqueries on
...
### Task
Read the task definitions in `task.sql` and replace each highlighted fragment so that the queries return the
required result.

...
So, what logically happens if we write something like this:
...
only inhabited planets, and then feeds the output to `GROUP BY`. Grouping evaluates the
...
Why we can't select the value of a non-grouping and non-aggregated column? 
...
Worth noticing that there are engines which are more optimistic and which allow for using non-grouping columns in 
`SELECT` from grouped table, hoping that programmers won't shoot their own foot. 
When a non-grouping column is in a select list of a query with `GROUP BY` clause, they choose a value from essentially random row from a group. 
...
we'll see that actually it won't do what it promises in the comment. 
It counts the flights made by those spacecrafts that had at least one flight. 
If some spacecraft had no flights, it will be missing. 
...
-- count the spacecrafts with flights.
...
-- This work as well: we applied an aggregate function to name column. 
...
Let's forget why one would write such query -- there might be many reasons -- but just imagine that our goal is 
...
Chances are that the goal was to find the id of a planet with maximum radius for each climate, and such task 
